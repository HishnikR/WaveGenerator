...
def test_phase_fourier():
    ...
    for i in range(90):
        ...
        for j in range(0, (round(PERIODS * fs / f0))):
            ...
            wav_re_shift += x * round(wampl * mp.cos(phase_shift + 2.0 * math.pi * (j - (round(PERIODS / 2 * fs / f0))) * f0 / fs))
            wav_im_shift += x * round(wampl * mp.sin(phase_shift + 2.0 * math.pi * (j - (round(PERIODS / 2 * fs / f0))) * f0 / fs))
        ...
        delta_shift = math.atan(wav_im_shift / wav_re_shift) - phi - phase_shift

        delta_sqr += sqrt(delta * delta / 90)
        print(i, delta, delta_shift)
